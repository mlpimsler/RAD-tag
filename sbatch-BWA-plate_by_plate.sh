...
do
        ...
        if [ -d "$DIR" ]
                ...
                for FILE in *.fq
                        ...
                        -t 4 -p $REFERENCE_SEQ $FILE > aligned-$FILENAME.sam
                        ...
                        #  --windowSize N [any two SNP calls and/or high entropy positions are considered clustered when they occur no more than 
                        #               N basepairs apart; default=10]
                        #  --mismatchFraction f [fraction of total sum of base qualities at a position that need to mismatch for the position to 
                        #               be considered to have high entropy; default=0.15; to disable, set to <= 0 or > 1]
                        #       Note that this fraction should be adjusted based on your particular data set. For deep coverage and/or when looking for
                        #               indels with low allele frequency, this number should be smaller.
                ...
                mv realigned* /grps1/jlozier/$USER/Alignments/BWA/RealignBAM/$(basename "$DIR")/.
                mv *.intervals /grps1/jlozier/$USER/Alignments/BWA/RealignBAM/Intervals/$(basename "$DIR")/.
